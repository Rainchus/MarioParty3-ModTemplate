...
N64CRC = os.path.join(SCRIPT_DIR, f"tools/n64crc{EXE_EXT}")
...
def create_main_asm(baseromName, romModName):
    header = f"""
...
def create_ninja_file(romModName, baseromName):
    ...
    with open('build.ninja', 'w') as buildfile:
        ...
        ninja.rule(
            "n64crc",
            command=f"{N64CRC} rom/{romModName}",
...
if __name__ == "__main__":
    ...
    print(f"[DEBUG] Running on {platform.system()}")
    print(f"[DEBUG] Using executable extension: '{EXE_EXT}'")
    print(f"[DEBUG] N64CRC tool path: {N64CRC}")
    
    create_ninja_file(romModName, baseromName)
    create_main_asm(baseromName, romModName)
